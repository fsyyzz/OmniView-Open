...
- **PDF 标注与高亮批注侧栏面板 (`PdfAnnotationsView`)**:
  - 新增完整的高亮批注抽屉侧栏，支持批注列表查看、便签文字编辑、点击跨页无缝跳转以及高亮条目管理；
  - 提供一键导出 Markdown 格式读书笔记能力（`exportAnnotationsAsMarkdown`），便于知识沉淀与二次整理；
  - 增强 PDF 文本层选区与高亮渲染（`PdfPageCanvas`）联动，支持黄色、绿色、粉色多色彩标记。
- **PDF 搜索体验优化与跨页定位增强 (`PdfSearchBar`)**:
  - 优化关键字检索与命中计数统计，支持回车键与快捷键无缝循环跳转上下文；
  - 强化视口自适应与缩放模式下的渲染同步。
