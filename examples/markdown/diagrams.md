...
## Domain Storytelling (领域故事讲授法 / egon.io)

在 Markdown 中支持嵌入 ````domainstory```` 代码块，原生渲染 DDD 业务领域故事流与参与者活动：

```domainstory
title: 快速审批办理故事流
1. 经办人员 (person) -> 提交报销凭证 -> 报销发票单 [document] -> 财务系统 (system)
2. 财务系统 -> 规则机自动初审 -> 审核校验报告 [data] -> 财务总监 (person)
3. 财务总监 -> 线上审批放行 -> 批复付款通知 [email] -> 资金结算中台 (system)
4. 资金结算中台 -> 执行银企直连打款 -> 汇款回单 [document] -> 经办人员
```

## 图表阅读建议

当图表内容较宽时，可以使用图表工具栏中的缩放按钮或直接双击画布进入全屏灯箱；需要修改语法时切换到源码模式，应用后即可重新渲染。还可以使用右上角播放按钮开启交互式逐帧演播。
